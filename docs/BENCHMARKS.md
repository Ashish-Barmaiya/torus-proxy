...
- **RAM:** 8.0 GB
- **OS:** Windows
- **Tool:** [Autocannon](https://github.com/mcollina/autocannon) — 100 concurrent connections, 10-second duration
- **Command:** `autocannon -c 100 -d 10 http://localhost:8080/api`

---

## Go Implementation Performance Analysis
...
- **Analysis:** With network network I/O roundtrips eliminated, this run captures the absolute processing limit of the Go runtime engine on a dual-core constraint, peaking at **15,644 req/sec**.
...
## Node.js Implementation (Archived — `node/` directory)
...
| Metric                | Node.js (Raw HTTP)              | Go (Native Torus Engine) | Improvement     |
| --------------------- | ------------------------------- | ------------------------ | --------------- |
| **Avg Throughput**    | 1,647 req/s                     | 6,044 req/s              | **~3.67×**      |
| **Avg Latency**       | —                               | 16.06 ms                 | —               |
| **P99 Latency**       | —                               | 63 ms                    | —               |
| **Concurrency Model** | 4 OS processes (`node:cluster`) | 1 process (goroutines)   | 10× less memory |
| **Memory Footprint**  | ~200 MB                         | ~20 MB                   | **~10×**        |

> **Note:** The Go benchmarks measure the proxy's own throughput (no backend running — requests are rejected immediately with 503/404). The Node.js benchmarks measured end-to-end throughput with live backends. A direct end-to-end comparison with Go + live backends + TLS + Redis is planned for a future version.

> On dedicated infrastructure (e.g., AWS c7g.large with 2+ physical cores), throughput scales linearly. With TLS and Redis rate limiting added on the Go side, throughput is conservatively expected in the **4,000–8,000 req/s** range on this same hardware.
